...
from .utils import MODULE2PACKAGE
from .utils import get_object_from_string, is_seq_of
            ...
            if default_scope is not None:
                ...
                try:
                    ...
                    if scope in MODULE2PACKAGE:
                        print(f'{scope} is not installed and its '
                        ...
                        print(f'Failed to import `{scope}.registry` '
    ...
    def import_from_location(self) -> None:
        ...
        if not self._imported:
            ...
            if len(self._locations) == 0 and self.scope in MODULE2PACKAGE:
                print(f'The "{self.name}" registry in {self.scope} did not '
                ...
                try:
                    ...
                    if self.scope in MODULE2PACKAGE:
                        print(f'{self.scope} is not installed and its '
                              'modules will not be registered. If you '
                              'want to use modules defined in '
                              f'{self.scope}, Please install {self.scope} by '
                              f'`pip install {MODULE2PACKAGE[self.scope]}.')
                    else:
                        print(f'Failed to import {self.scope} and register '
            ...
            for loc in self._locations:
                import_module(loc)
                print(f"Modules of {self.scope}'s {self.name} registry have "
    ...
    def get(self, key: str) -> Optional[Type]:
        ...
        if scope is None or scope == self._scope:
            ...
            try:
                ...
                print(f'Registry node of {scope} has been automatically '
                      'imported.')
            except (ImportError, AttributeError, ModuleNotFoundError):
                print(f'Cannot auto import {scope}.registry, please check '
        ...
        if obj_cls is not None:
            ...
            print(f'Get class `{cls_name}` from "{registry_name}"'
